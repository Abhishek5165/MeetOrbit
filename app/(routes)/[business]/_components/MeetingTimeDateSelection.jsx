...
function MeetingTimeDateSelection({ eventInfo, businessInfo }) {
  ...
  const sendEmail = (user) => {
    ...
    const emailHtml = ReactDOMServer.renderToString(<Email
      businessName={businessInfo?.businessName}
  ...
  return (
    ...
    >
      <Link href='/dashboard'><Image src='/logo-transparent.png' alt='logo'
       width={200}
       height={200}/></Link>
      <div className="grid grid-cols-1 md:grid-cols-3 mt-5">
        {/* Meeting Info  */}
        <div className="p-4 border-r mt-3">
          ...
          <div className="mt-5 flex flex-col gap-4">
            ...
            <h2 className="flex gap-2">
              <CalendarCheck />
              {date ? format(date, "PPP") : "Invalid Date !"}
            </h2>
            {selectedTime && (
              <h2 className="flex gap-2">
                <Timer />
